...
All `@cuilabs/qnsp-*` SDKs below are TypeScript-first, ESM, and published to the public npm registry under **Apache-2.0**. They install cleanly from a default `.npmrc` with **no GitHub Packages scope configuration required**.

**Runtime floor:** every manifest declares `engines.node >= 22.0.0`.

**Post-quantum backend:** the PQC primitives in `@cuilabs/qnsp-cryptography` resolve against the pure-JS [`@noble/post-quantum`](https://github.com/paulmillr/noble-post-quantum) provider by default. The optional native backend (`@cuilabs/liboqs-native`) is published to GitHub Packages only and declared as an `optionalDependency` of `@cuilabs/qnsp-cryptography` — npm will skip it silently on public installs. To opt in, add a scope mapping and install it explicitly:
...
|---------|---------|---------|
| `@cuilabs/qnsp-auth-sdk` | 0.3.6 | `pnpm add @cuilabs/qnsp-auth-sdk` |
| `@cuilabs/qnsp-vault-sdk` | 0.3.9 | `pnpm add @cuilabs/qnsp-vault-sdk` |
| `@cuilabs/qnsp-kms-client` | 0.2.6 | `pnpm add @cuilabs/qnsp-kms-client` |
| `@cuilabs/qnsp-storage-sdk` | 0.3.6 | `pnpm add @cuilabs/qnsp-storage-sdk` |
| `@cuilabs/qnsp-search-sdk` | 0.2.10 | `pnpm add @cuilabs/qnsp-search-sdk` |
| `@cuilabs/qnsp-audit-sdk` | 0.3.6 | `pnpm add @cuilabs/qnsp-audit-sdk` |
| `@cuilabs/qnsp-billing-sdk` | 0.2.6 | `pnpm add @cuilabs/qnsp-billing-sdk` |
| `@cuilabs/qnsp-tenant-sdk` | 0.3.6 | `pnpm add @cuilabs/qnsp-tenant-sdk` |
| `@cuilabs/qnsp-access-control-sdk` | 0.3.6 | `pnpm add @cuilabs/qnsp-access-control-sdk` |
| `@cuilabs/qnsp-crypto-inventory-sdk` | 0.3.6 | `pnpm add @cuilabs/qnsp-crypto-inventory-sdk` |
| `@cuilabs/qnsp-ai-sdk` | 0.1.11 | `pnpm add @cuilabs/qnsp-ai-sdk` |
| `@cuilabs/qnsp-browser` | 0.1.4 | `pnpm add @cuilabs/qnsp-browser` |
| `@cuilabs/qnsp-sdk-activation` | 0.1.6 | `pnpm add @cuilabs/qnsp-sdk-activation` |

### Framework adapters

| Package | Version | Install |
|---------|---------|---------|
| `@cuilabs/qnsp-langchain-qnsp` | 0.1.7 | `pnpm add @cuilabs/qnsp-langchain-qnsp` |
| `@cuilabs/qnsp-llamaindex-qnsp` | 0.2.5 | `pnpm add @cuilabs/qnsp-llamaindex-qnsp` |
| `@cuilabs/qnsp-autogen-qnsp` | 0.2.5 | `pnpm add @cuilabs/qnsp-autogen-qnsp` |

### Bin-first packages

| Package | Version | Install | Bin |
|---------|---------|---------|-----|
| `@cuilabs/qnsp-cli` | 0.1.12 | `pnpm add -g @cuilabs/qnsp-cli` | `qnsp` |
| `@cuilabs/qnsp-mcp` | 0.1.3 | `pnpm add -g @cuilabs/qnsp-mcp` | `qnsp-mcp` |

### Core libraries (consumed by the SDKs above)

| Package | Version | Purpose |
|---------|---------|---------|
| `@cuilabs/qnsp-cryptography` | 0.2.0 | PQC primitives (ML-KEM, ML-DSA, SLH-DSA) via noble; optional liboqs native backend |
| `@cuilabs/qnsp-shared-kernel` | 0.1.4 | Domain primitives, JWT helpers, error hierarchy, SDK-facing tier gating |
| `@cuilabs/qnsp-observability` | 0.1.4 | OpenTelemetry helpers, structured logging |
| `@cuilabs/qnsp-events` | 0.1.4 | Event envelope schemas |
| `@cuilabs/qnsp-resilience` | 0.1.2 | Retry / circuit-breaker primitives |
...
`@cuilabs/qnsp-shared-kernel` exports a small, **self-contained** tier catalogue so SDK consumers can fail fast with a typed error before making a network call:
...
The catalogue is **inlined** in `@cuilabs/qnsp-shared-kernel` — no separate pricing package is required. The internal `@cuilabs/qnsp-pricing` package is private to this monorepo and deliberately not published; a build-time drift test (`packages/shared-kernel/src/tier-limits.drift.test.ts`) keeps the inlined catalogue byte-exact with the internal commercial source of truth.
