/**
 * Smoke tests for @cuilabs/qnsp-langchain-qnsp
 *
 * Verifies that the package exports are importable and classes can be instantiated.
 * No real API key or network access required — `@cuilabs/qnsp-sdk-activation` is mocked
 * so the activation handshake resolves immediately. Catches build regressions.
 *
 * Run: pnpm --filter @cuilabs/qnsp-langchain-qnsp test
 */
